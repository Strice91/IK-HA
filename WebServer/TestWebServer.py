...
@author: Maximilian Pautzke and Andreas Blenk

Code for testing the correctness of an implementation of the Web Server. All 
functions are tested one after another Code returns an error if a request is 
...
# create formatter
formatter = logging.Formatter('[%(asctime)s - %(name)s - %(levelname)s - Method: %(funcName)s] \n    %(message)s')
...
def compare(m1, m2, self):
    """ 
    this function makes sure the server name in a http response can be 
    ...
    message = "Anything is wrong with your statusline!"
    self.assertEqual(m1_array[0], m2_array[0], message)  # compare statusLine
    
    message = "Anything is wrong with your headerline!"
    self.assertEqual(m1_array[2], m2_array[2], message)  # compare headerLines (ContentType)
    
    message = "Anything is wrong with your headerline!"
    self.assertEqual(m1_array[3], m2_array[3], message)  # compare headerLines (blank)
    
    # self.assertEqual(m1_array[4], m2_array[4])  # compare blankLine
    # self.assertEqual(m1_array[5], m2_array[5])  # compare entityBody

class Test(unittest.TestCase):
    def test_version(self):
        self.assertTrue(sys.version > '3', 'You are using the wrong python version!')
    ...
    def test_response404_NotFound(self):
        ...
        entityBodyTestString = "<html><head></head><body><h1>404 Not Found</h1>"
        ...
        message = "Your body seams to be wrong!"
    ...
    def test_response200_OK(self):
        ...
        # test whether equal
        message = "Message is not correct! Anything is missing ... or too much?"
    ...
    def test_parseMessage_Wrong(self):
        ...
        for message in messages:
            ...
            strMessageFromWebserverToTest = bytes.decode(message_from_webserver)
    ...
    def test_parseMessageCorrect(self):
        ...
        self.assertFalse(bytes("404", encoding="utf-8") in testMessageBytesWebserver)
        self.assertNotEqual(testMessageBytesWebserver, correctMessageBytesWebserver)

        logger.info('JFIF' in str(testMessageBytesWebserver))
        debug_message = "Did you really load a JPG? There is something missing."
        self.assertTrue('JFIF' in str(testMessageBytesWebserver), debug_message)
        ...
        self.assertFalse(bytes("404", encoding="utf-8") in testMessageBytesWebserver, debug_message)
        self.assertNotEqual(testMessageBytesWebserver, incorrectMessageBytesWebserver)

        # Serach for jpg bytes in received data
        jpgBytes = '\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x01\x00/\x00/\x00'
        self.assertFalse('JFIF' in str(testMessageBytesWebserver))
        self.assertFalse(jpgBytes in str(testMessageBytesWebserver))


# some constants for code simplification
blankLine = "\r\n"
 
if __name__ == "__main__":
    """
    if Test_Webserver_ML is main function start the test procedure
    import sys;sys.argv = ['', 'Test.test_404']
    """
    logger.debug("Webserver is being tested...")
